...
import {createPost, deletePost, updatePostByID} from '@/app/lib/actions/posts';
...
import {deleteFromVps, uploadToVps} from '@/app/lib/actions/file';
...
export default function CreateUpdatePost({
    ...
    locale: Locales;
    isAdd: boolean;
    data: Omit<PostInterface, '_id' | 'date' | 'suggested_blob_paths' | 'createdAt' | 'updatedAt'>;
}) {
    ...
    const [values, setValues] = useState<Omit<PostInterface, '_id' | 'date' | 'suggested_blob_paths' | 'createdAt' | 'updatedAt'>>({
        ...structuredClone(data),
    });
    const [am, setAm] = useState(values.content.am);
    const [ru, setRu] = useState(values.content.ru);
    const [en, setEn] = useState(values.content.en);
    ...
    return (<div className={styles.page}>
        ...
        <div className={styles.header}>
            <h3 className={styles.title}>
                {isAdd ? t('Add post') : !data.title[locale] ? data.title['am'] : data.title[locale]}
            </h3>
        ...
        <div className={styles.section}>
            ...
            <label className={styles.inputContainer}>
                ...
                <div className={styles.multilangualInputs}>
                    <TextField
                        ...
                        helperText={!values.title.am ? t('Title in Armenian is required') : ''}
                        value={values.title.am}
                        onChange={(e) => {
                            setValues({
                                ...values, title: {...values.title, am: e.target.value},
                            });
                        }}
                    />
                    <TextField
                        ...
                        onChange={(e) => {
                            setValues({
                                ...values, title: {...values.title, en: e.target.value},
                            });
                        }}
                    />
                    <TextField
                        ...
                        onChange={(e) => {
                            setValues({
                                ...values, title: {...values.title, ru: e.target.value},
                            });
                        }}
                    />
                </div>
            ...
            <div className={styles.multilangualInputs}>
                <label className={styles.inputContainer}>
                    ...
                    <TextField
                        ...
                        required
                        error={!values.slug}
                        helperText={!values.slug ? t('Slug is required') : t('Unique identifier for the post, used in the URL')}
                        value={values.slug}
                        onChange={(e) => {
                            setValues({
            ...
            <label className={styles.inputContainer}>
                ...
                <LocalizationProvider dateAdapter={AdapterDayjs} adapterLocale={'ru'}>
                    <DateTimePicker
                        ...
                        onChange={(e) => {
                            if (!e) return;
                            setValues({...values, publishing_date: e.toDate()});
                        }}
                        ampm={false}
                        referenceDate={dayjs()}
                    />
            ...
            <label className={styles.inputContainer}>
                ...
                <div className={styles.multilangualInputs}>
                    ...
                    <Button
                        ...
                        onClick={() => {
                            if (!!hashtags) {
                                setValues({
                                    ...values, hashtags: [...values.hashtags, hashtags],
                                });
                                setHashtags('');
                            }
                        }}
                ...
                {values.hashtags.length > 0 && (<div className={styles.hashtags}>
                    {values.hashtags.map((h) => (<div className={styles.tag} key={h}>
                        {h}
                        <IconButton
                            onClick={() => {
                                setValues({
                                    ...values, hashtags: values.hashtags.filter((item) => item !== h),
                                });
                            }}
                        >
                            <CloseIcon/>
