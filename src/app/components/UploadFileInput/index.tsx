...
export default function UploadFileInput({
  ...
  const notify = useCallback(
    (nextFiles: File[], nextOldFiles: string[]) => {
      onFilesChange?.(nextFiles, nextOldFiles);
    },
    [onFilesChange],
  );

  const validateAndBuildNext = useCallback(
  ...
  return (
    <div className={styles.container}>
      {oldFiles.length > 0 &&
        oldFiles.map((f) => (
          <div key={f} className={styles.imageContainer}>
