...
export async function getTopPost({
  ...
  if (s) {
    filter.$or = [
      { "title.en": { $regex: s, $options: "i" } },
      { "title.am": { $regex: s, $options: "i" } },
      { "title.ru": { $regex: s, $options: "i" } },
      { hashtags: { $in: [new RegExp(s, "i")] } },
    ];
  }
  filter.date = { $lt: new Date().toISOString(), $gte: date.toISOString() };
  ...
  const [items, total] = await Promise.all([
    Contents.find(filter, projection)
      .sort({ date: dir })
      .skip((page - 1) * limit)
      .limit(limit)
      .lean(),
  ...
  return {
    data: items as unknown as PostInterface[],
...
export async function getMostViewedPost({
  ...
  const data = {
    data: JSON.parse(JSON.stringify(topPost)),
