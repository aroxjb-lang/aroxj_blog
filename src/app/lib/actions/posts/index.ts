...
export async function getTopPost({
	...
	const filter: any = {
		$and: [{$or: [{publishing_date: {$exists: false}}, {publishing_date: {$lt: new Date()}}]}],
	};
...
export async function getArchive({
	...
	await Promise.all(
		items.map(async (item) => {
			const firstImage = item.featured_media_paths?.[0];
			...
			await Contents.findOneAndUpdate(
				{ _id: item.id },
				{
					$set: {
						...
						featured_media_paths: firstImage
							? [
								firstImage.includes("wp-content/")
									? firstImage
									: `wp-content/${firstImage}`,
							]
							: [],
					},
				}
